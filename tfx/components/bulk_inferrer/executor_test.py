...
class ExecutorTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self._output_example = standard_artifacts.Examples()
    self._output_example_dir = os.path.join(self._output_data_dir,
                                            'output_example')
    self._output_example.uri = self._output_example_dir
  ...
  def _varify_example_split(self, split_name):
    self.assertTrue(
        fileio.exists(os.path.join(self._output_example_dir, split_name)))
    results = self._get_results(
        os.path.join(self._output_example_dir, split_name),
  ...
  def testDoWithOutputExampleAllSplits(self):
    ...
    # Check outputs.
    self.assertTrue(fileio.exists(self._output_example_dir))
    self._varify_example_split('unlabelled')
    self._varify_example_split('unlabelled2')
  ...
  def testDoWithOutputExampleSpecifiedSplits(self):
    ...
    self._varify_example_split('unlabelled')
    self.assertFalse(
        fileio.exists(os.path.join(self._output_example_dir, 'unlabelled2')))
