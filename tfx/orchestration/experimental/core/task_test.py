...
class TaskTest(tu.TfxTest):
  ...
  def test_task_type_ids(self):
    ...
    self.assertEqual('RegisterPipelineTask',
                     task_lib.RegisterPipelineTask.task_type_id())
    self.assertEqual('UnregisterPipelineTask',
                     task_lib.UnregisterPipelineTask.task_type_id())
  ...
  def test_task_ids(self):
    ...
    register_pipeline_task = task_lib.RegisterPipelineTask(
        pipeline_uid=pipeline_uid,
        pipeline=pipeline_pb2.Pipeline(),
        callback=lambda: None)
    self.assertEqual(('RegisterPipelineTask', pipeline_uid),
                     register_pipeline_task.task_id)
    unregister_pipeline_task = task_lib.UnregisterPipelineTask(
        pipeline_uid=pipeline_uid, callback=lambda: None)
    self.assertEqual(('UnregisterPipelineTask', pipeline_uid),
                     unregister_pipeline_task.task_id)
