...
_EXAMPLES_FILE_NAME = 'data_tfrecord'
...
class Executor(base_executor.BaseExecutor):
  ...
  def _set_inferred_property(self, output_example: Optional[types.Artifact],
                             inference_result: Optional[types.Artifact],
                             inferred: int) -> None:
    if inference_result:
      inference_result.set_int_custom_property('inferred', inferred)
    if output_example:
      output_example.set_int_custom_property('inferred', inferred)
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    if output_dict.get('output_example'):
      output_example = artifact_utils.get_single_instance(
    ...
    if 'model_blessing' in input_dict:
      ...
      if not model_utils.is_model_blessed(model_blessing):
        self._set_inferred_property(inference_result, output_example, 0)
    ...
    self._run_model_inference(
        data_spec, output_example_spec, input_dict['examples'], output_example,
        inference_result, self._get_inference_spec(model_path, exec_properties))
    self._set_inferred_property(inference_result, output_example, 1)
  ...
  def _run_model_inference(
      ...
      output_example: Optional[types.Artifact],
      inference_result: Optional[types.Artifact],
      inference_endpoint: model_spec_pb2.InferenceSpecType,
  ) -> None:
    """Runs model inference on given example data.
    ...
    Args:
      ...
      output_example: Optional output example artifact.
    ...
    if output_example:
      output_example.split_names = artifact_utils.encode_split_names(
          list(example_uris.keys()))
    ...
    with self._make_beam_pipeline() as pipeline:
      ...
      for split, example_uri in example_uris.items():
        ...
        if output_example:
          output_example_split_uri = artifact_utils.get_split_uri(
              [output_example], split)
          logging.info('Path of output example split `%s` is %s.', split,
                       output_example_split_uri)
          _ = (
              data
              | 'WriteExample[{}]'.format(split) >> _WriteExample(
                  output_example_spec, output_example_split_uri))
    ...
    if output_example:
      logging.info('Output example written to %s.', output_example.uri)
    if inference_result:
