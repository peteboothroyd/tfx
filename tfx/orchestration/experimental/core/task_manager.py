...
class TaskManager:
  ...
  def _handle_task(self, task: task_lib.Task) -> None:
    ...
    elif task_lib.is_register_pipeline_task(task):
      self._handle_register_pipeline_task(
          typing.cast(task_lib.RegisterPipelineTask, task))
    elif task_lib.is_unregister_pipeline_task(task):
      self._handle_unregister_pipeline_task(
          typing.cast(task_lib.UnregisterPipelineTask, task))
    else:
  ...
  def _handle_exec_node_task(self, task: task_lib.ExecNodeTask) -> None:
    ...
    with self._tm_lock:
      ...
      scheduler = ts.TaskSchedulerRegistry.create_task_scheduler(
          self._mlmd_handle, self._pipeline_by_uid[task.node_uid.pipeline_uid],
          task)
  ...
  def _handle_register_pipeline_task(
      self, task: task_lib.RegisterPipelineTask) -> None:
    """Handles `RegisterPipelineTask`."""
    with self._tm_lock:
      self._pipeline_by_uid[task.pipeline_uid] = task.pipeline
      task.callback()
      self._task_queue.task_done(task)

  def _handle_unregister_pipeline_task(
      self, task: task_lib.UnregisterPipelineTask) -> None:
    """Handles `UnregisterPipelineTask`."""
    with self._tm_lock:
      self._pipeline_by_uid.pop(task.pipeline_uid, None)
      task.callback()
      self._task_queue.task_done(task)
  ...
  def _process_exec_node_task(self, scheduler: ts.TaskScheduler,
    ...
    # a failed execution and MLMD updated accordingly.
    try:
