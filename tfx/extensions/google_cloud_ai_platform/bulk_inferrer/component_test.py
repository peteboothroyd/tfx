...
class ComponentTest(tf.test.TestCase):
  ...
  def testConstructInferenceResult(self):
    ...
    self.assertNotIn('output_example', bulk_inferrer.outputs.keys())
  ...
  def testConstructOutputExample(self):
    ...
    self.assertEqual('Examples',
                     bulk_inferrer.outputs['output_example'].type_name)
    self.assertNotIn('inference_result', bulk_inferrer.outputs.keys())

  def testConstructInferenceResultAndOutputExample(self):
    with self.assertRaises(ValueError):
      _ = component.CloudAIBulkInferrerComponent(
          examples=self._examples,
          model=self._model,
          model_blessing=self._model_blessing,
          output_example=channel_utils.as_channel(
