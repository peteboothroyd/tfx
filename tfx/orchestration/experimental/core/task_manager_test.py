  ...
  def test_task_handling(self, mock_publish):
    ...
    task_queue.enqueue(
        task_lib.RegisterPipelineTask(
            pipeline_uid=task_lib.PipelineUid.from_pipeline(self._pipeline),
    ...
    with self._task_manager(task_queue) as task_manager:
      ...
      unregister_pipeline_task = task_lib.UnregisterPipelineTask(
          pipeline_uid=task_lib.PipelineUid.from_pipeline(self._pipeline),
          callback=lambda: None)
      task_queue.enqueue(unregister_pipeline_task)

    self.assertTrue(task_manager.done())
    self.assertIsNone(task_manager.exception())
    self.assertEmpty(task_manager._pipeline_by_uid)
...
class TaskManagerE2ETest(tu.TfxTest):
  ...
  def _run_task_manager(self):
    with self._mlmd_connection as m:
      task_manager = tm.TaskManager(m, self._task_queue, 1000)
      with task_manager:
        pass
    return task_manager
